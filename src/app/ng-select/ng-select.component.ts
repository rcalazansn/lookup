...
import { distinctUntilChanged, debounceTime, switchMap, tap, catchError, takeUntil, startWith, map, filter } from 'rxjs/operators'
...
export class NgSelectComponent implements OnDestroy, OnChanges, AfterViewInit, ControlValueAccessor {
    ...
    @Input() showModalText :string = "Filtro";
    ...
    @Output('showFilter') showFilterEvent = new EventEmitter();
    ...
    handleShowModalClick($event: Event) {
        $event.stopPropagation();
